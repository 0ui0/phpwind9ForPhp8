...
//    echo "连接数据库...<br>";
    ...
    if (!$con) showError('Could not connect: ' . mysqli_connect_error());
//    echo "设置查询字符集...<br>";
    $result = mysqli_query($con, "SET character_set_connection= 'utf8', character_set_results= 'utf8', character_set_client=BINARY, sql_mode=''") or die("Invalid query: " . mysqli_error($con));
    $result = mysqli_query($con, "SET NAMES 'utf8'") or die("Invalid query: " . mysqli_error($con));
    //如果是二次运行，本次导入表情包前先删除旧数据
    $sql = "DELETE FROM `{$dbpre}common_emotion_category` WHERE `emotion_folder`='wangwang'";
    mysqli_query($con, $sql) or showError("Invalid query: " . mysqli_error($con));
    ...
    $result = mysqli_query($con, $sql) or showError("Invalid query: " . mysqli_error($con));
