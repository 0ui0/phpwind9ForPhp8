...
//private标记的构造方法
    ...
    private function __construct() {
        ...
        //    echo "连接数据库...<br>";
        $this->con = mysqli_connect($host, $username, $password, $dbname, $port);
        if (!$this->con) {
            showError('Could not connect: ' . mysqli_connect_error());
        }
        //    echo "设置查询字符集...<br>";
        mysqli_query($this->con, "SET character_set_connection= 'utf8', character_set_results= 'utf8', character_set_client=BINARY, sql_mode=''") or die("Invalid query: " . mysqli_error($this->con));
        mysqli_query($this->con, "SET NAMES 'utf8'") or die("Invalid query: " . mysqli_error($this->con));
    }
...
//单例方法,用于访问实例的公共的静态方法
    ...
    public function query($sql) {
        $result = mysqli_query($this->con, $sql) or die("Invalid query: " . mysqli_error($this->con));
        $res = array();
...
//    echo "检查扩展是否缺少...<br>";
    ...
    if(!in_array("openssl", $extensions))showError("缺少openssl扩展");
...
//    echo "查找站点一级版面...<br>";
    ...
    $result = mysqli_query($pw_db->con, $sql) or showError("Invalid query: " . mysqli_error($pw_db->con));
    $fids = array();
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
        $fids[$row['fid']] = 0;
    }
