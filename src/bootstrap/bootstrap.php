		...
		foreach ($_consts as $const => $value) {
			if ($const === 'PUBLIC_URL') {
				if (!$value && !$realPublicUrl) {
					...
					$realPublicUrl = $value;
				} elseif ($realPublicUrl) {
					$value = $realPublicUrl;
				}
				if (!defined('PUBLIC_URL')) define('PUBLIC_URL', $value);
				continue;
			}
			if (defined($const)) continue;
			if ($const !== 'PUBLIC_URL' && $value && $value[0] === '/' && $realPublicUrl) {
				$value = rtrim($realPublicUrl, '/') . $value;
			}
			define($const, $value);
		}
		$url = new stdClass();
		$url->base = $realPublicUrl;
