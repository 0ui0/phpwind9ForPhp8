...
class IndexController extends WindController {
	...
	public function dataAction() {
		...
		$logFile = '/Users/lambda/Desktop/phpwind/data/install_debug.log';
		file_put_contents($logFile, "Starting dataAction at " . date('Y-m-d H:i:s') . "\n", FILE_APPEND);
		...
		try {
			...
			$tableSql = include $this->_getTableSqlFile();
			try {
				if (isset($tableSql['UPDATE']) && is_array($tableSql['UPDATE'])) {
					file_put_contents($logFile, "Executing UPDATE SQLs...\n", FILE_APPEND);
					foreach ($tableSql['UPDATE'] as $sql) {
						$pdo->query($sql);
					}
					file_put_contents($logFile, "UPDATE SQLs executed.\n", FILE_APPEND);
				}
			} catch (PDOException $e) {
				file_put_contents($logFile, "SQL Execution Error: " . $e->getMessage() . "\n", FILE_APPEND);
				$this->showError($e->getMessage(), false);
			}
			$pdo->close();
			...
			file_put_contents($logFile, "WindForwardException caught (Redirecting)...\n", FILE_APPEND);
			throw $e;
		} catch (\Throwable $e) {
			$msg = "Error in dataAction: " . $e->getMessage() . "\n" .
				   "File: " . $e->getFile() . "\n" .
				   "Line: " . $e->getLine() . "\n" .
				   "Trace:\n" . $e->getTraceAsString();
	...
	private function _sqlParser($strSQL, $charset, $dbprefix, $engine) {
		...
		foreach ($arrSQL as $value) {
			$value = trim($value, " \t");
			if (!$value || substr($value, 0, 2) === '--' || substr($value, 0, 2) === '/*') continue;
			$query .= $value;
			...
			if ($sql_key == 'CREATE') {
				...
				// Force add DROP TABLE IF EXISTS to ensure clean install
				$dataSQL['DROP'][] = "DROP TABLE IF EXISTS `$tablename`";
			} elseif ($sql_key == 'DROP') {
