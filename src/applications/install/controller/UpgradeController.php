...
// 		WindFile::del(Wind::getRealPath('DATA:setup.tmp_dbsql.php', true));
	...
	private function getMaxUid() {
		...
		$link = mysqli_connect($db_config['src_host'], $db_config['src_username'], $db_config['src_password'], $db_config['src_dbname'], $db_config['src_port']);
		...
		if (false === $rt) {
			$this->showError('SQL ERROR:' . mysqli_error($link) . ' IN "' . $sql . '"');
		}
		$result = mysqli_fetch_array($rt, MYSQLI_NUM);
		if (!$result[0]) {
