...
// 		$info = $this->_getUserDs()->getUserByUid($uid, PwUser::FETCH_MAIN);
		...
		if (!$this->loginUser->isExists() || Pw::getPwdCode($this->loginUser->info['password']) != $password) {
            // Debug logging
            $log = "CheckUserInfo Failed.\nUID: $uid\nExists: " . ($this->loginUser->isExists() ? 'Yes' : 'No') . "\n";
            if ($this->loginUser->isExists()) {
                $log .= "DB Pwd: " . $this->loginUser->info['password'] . "\n";
                $log .= "Calc PwdCode: " . Pw::getPwdCode($this->loginUser->info['password']) . "\n";
                $log .= "Token PwdCode: " . $password . "\n";
            }
            file_put_contents(Wind::getRealPath('DATA:log.login_error.txt'), date('Y-m-d H:i:s') . "\n" . $log . "\n", FILE_APPEND);
