...
class PwNavBo {
	...
	protected function routeRating() {
		$rating = array();
		$rating[] = '';
		$m = isset($this->router['m']) ? $this->router['m'] : '';
		$c = isset($this->router['c']) ? $this->router['c'] : '';
		$a = isset($this->router['a']) ? $this->router['a'] : '';
		$rating[] = $m .'|'. $c .'|'. $a.'|';
    	if ($m == 'bbs') { //帖子兼容
    		$rating[] = $m .'|'. $c .'|'. $a .'|'.(isset($this->forum['tid']) ? $this->forum['tid'] : '').'|';
    		$rating[] = $m .'|thread|'. $this->default['a'] .'|'.(isset($this->forum['fid']) ? $this->forum['fid'] : '').'|';
    		if ($c == 'cate') {
    			$rating[] = $m .'|cate|'. $this->default['a'] .'|'.(isset($this->forum['fid']) ? $this->forum['fid'] : '').'|';
    		} else {
    			$rating[] = $m .'|cate|'. $this->default['a'] .'|'.(isset($this->forum['cateid']) ? $this->forum['cateid'] : '').'|';
    		}
		...
		if (!is_array($home)) $home = array('m'=>'', 'c'=>'', 'a'=>'');
    	$rating[] = (isset($home['m']) ? $home['m'] : '') .'|'. (isset($home['c']) ? $home['c'] : '') .'|'. (isset($home['a']) ? $home['a'] : '').'|';
