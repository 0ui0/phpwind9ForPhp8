...
class PwUbbCode {
	...
	public static function convertSize($message, $maxSize = 0) {
		$message = preg_replace_callback("/\[size=(\d+)\]/is", function($matches) use ($maxSize) {
			return self::_size($matches[1], $maxSize);
		}, $message);
	...
	public static function convertTable($message, $max = 0) {
		...
		while (self::hasTag($message, 'table')) {
			$message = preg_replace_callback('/\[table(?:=(\d{1,4}(?:%|px)?)(?:,(#\w{6})?)?(?:,(#\w{6})?)?(?:,(\d+))?(?:,(\d+))?(?:,(left|center|right))?)?\](?!.*(\[table))(.*?)\[\/table\]/is', function($matches) {
				return self::createTable($matches[8], $matches[1], $matches[2], $matches[3], $matches[4], $matches[5], $matches[6]);
			}, $message);
			if (++$t > $max) break;
		}
		return $message;
	...
	public static function parseEmotion($message) {
		$message = preg_replace_callback("/\[s:(.+?)\]/is", function($matches) {
			return self::createEmotion($matches[1]);
	...
	public static function parseAttachment($message, $config) {
		return $message = preg_replace_callback('/\[(attachment|p_w_upload|p_w_picpath)=(\d+)\]/is', function($matches) use ($config) {
			return self::createAttachment($matches[2], $config);
		}, $message);
	}
	...
	public static function parseImg($message, $convertStatus = 1, $maxWidth = 0, $maxHeight = 0, $isLazy = false) {
		if ($convertStatus) {
			return preg_replace_callback("/\[img\]([^\<\(\r\n\"']+?)\[\/img\]/is", function($matches) use ($maxWidth, $maxHeight, $isLazy) {
				return self::createImg($matches[1], $maxWidth, $maxHeight, '', $isLazy);
			}, $message, self::$_cvtimes);
		}
		return preg_replace_callback("/\[img\]([^\<\(\r\n\"']+?)\[\/img\]/is", function($matches) {
			return self::createImgLink($matches[1]);
		}, $message, self::$_cvtimes);
	}
	...
	public static function parseUrl($message, $checkurl = 0) {
		$message = preg_replace_callback("/\[url=((https?|ftp|gopher|news|telnet|mms|rtsp|thunder|ed2k)?[^\[\s]+?)(\,(1)\/?)?\](.+?)\[\/url\]/is", function($matches) use ($checkurl) {
			return self::createUrl($matches[1], $matches[5], $matches[2], $matches[4], $checkurl);
		}, $message);

		$message = preg_replace_callback("/\[url\]((https?|ftp|gopher|news|telnet|mms|rtsp|thunder|ed2k)?[^\[\s]+?)\[\/url\]/is", function($matches) use ($checkurl) {
			return self::createUrl($matches[1], $matches[1], $matches[2], '0', $checkurl);
		}, $message);
	...
	public static function parseCode($message) {
		return preg_replace_callback("/\[code(\s*brush\:(.+?)\;toolbar\:(true|false)\;)?\](.+?)\[\/code\]/is", function($matches) {
			return self::createCode($matches[4], $matches[2], $matches[3]);
		}, $message, self::$_cvtimes);
	}
	...
	public static function parsePost($message, $config) {
		return preg_replace_callback("/\[post\](.+?)\[\/post\]/is", function($matches) use ($config) {
			return self::createPost($matches[1], $config);
		}, $message);
	}
	...
	public static function parseHide($message, $config) {
		return preg_replace_callback("/\[hide=(.+?)\](.+?)\[\/hide\]/is", function($matches) use ($config) {
			return self::createHide($matches[1], $matches[2], $config);
		}, $message);
	}
	...
	public static function parseSell($message, $config) {
		return preg_replace_callback("/\[sell=(\d+)(\,(\d+))?\](.+?)\[\/sell\]/is", function($matches) use ($config) {
			return self::createSell($matches[1], $matches[3], $matches[4], $config);
		}, $message);
	}
	...
	public static function parseQuote($message) {
		return preg_replace_callback("/\[quote(=(.+?)\,(\d+))?\](.*?)\[\/quote\]/is", function($matches) {
			return self::createQoute($matches[4], $matches[2], $matches[3]);
		}, $message);
		/** 盖楼
	...
	public static function parseFlash($message, $convertStatus = 1) {
		if ($convertStatus) {
			return preg_replace_callback("/\[flash(=(\d+?)\,(\d+?)(\,(0|1))?)?\]([^\[\<\(\r\n\"']+?)\[\/flash\]/is", function($matches) {
				return self::createPlayer($matches[6], $matches[2], $matches[3], $matches[5], 'video');
			}, $message, self::$_cvtimes);
		}
		return preg_replace_callback("/\[flash(=(\d+?)\,(\d+?)(\,(0|1))?)?\]([^\[\<\(\r\n\"']+?)\[\/flash\]/is", function($matches) {
			return self::createFlashLink($matches[6]);
		}, $message, self::$_cvtimes);
	}
	...
	public static function parseMedia($message, $convertStatus = 1) {
		if ($convertStatus) {
			$message = preg_replace_callback("/\[(wmv|mp3)(=(0|1))?\]([^\<\(\r\n\"']+?)\[\/\\1\]/is", function($matches) {
				return self::createPlayer($matches[4], '314', '53', $matches[3], 'audio');
			}, $message, self::$_cvtimes);
			
			$message = preg_replace_callback("/\[(wmv|rm)(=([0-9]{1,3})\,([0-9]{1,3})\,(0|1))?\]([^\<\(\r\n\"']+?)\[\/\\1\]/is", function($matches) {
				return self::createPlayer($matches[6], $matches[3], $matches[4], $matches[5], 'video');
			}, $message, self::$_cvtimes);
			return $message;
		}
		
		$message = preg_replace_callback("/\[(mp3|wmv)(?:=[01]{1})?\]([^\<\r\n\"']+?)\[\/\\1\]/is", function($matches) {
			return self::createMediaLink($matches[2]);
		}, $message, self::$_cvtimes);
		
		$message = preg_replace_callback("/\[(wmv|rm)(?:=[0-9]{1,3}\,[0-9]{1,3}\,[01]{1})?\]([^\<\r\n\"']+?)\[\/\\1\]/is", function($matches) {
			return self::createMediaLink($matches[2]);
		}, $message, self::$_cvtimes);
    ...
    public static function parseTao($message) {
		return preg_replace_callback("/\[tao=([^,]+),([^,]+),([^,]+),([^,]+),([^,]+)\]\[\/tao\]/is", function($matches) {
			return self::createTao($matches[1], $matches[4], $matches[3], $matches[2], $matches[5]);
		}, $message);
	}

    public static function parseShare($message) {
		return preg_replace_callback("/\[share=([^,]+),([^\]]*)\](.+?)\[\/share\]/is", function($matches) {
			return self::createShare($matches[1], $matches[2], $matches[3]);
		}, $message);
	}

	public static function parseRemind($message, $remindUser) {
		return preg_replace_callback('/@([\x7f-\xff\dA-Za-z\.\_]+)(?=\s?)/i', function($matches) use ($remindUser) {
			return self::createRemind($matches[1], $remindUser);
		}, $message);
	}
	...
	public static function parseIframe($message, $convertStatus = 1) {
		return preg_replace_callback("/\[iframe\]([^\[\<\(\r\n\"']+?)\[\/iframe\]/is", function($matches) use ($convertStatus) {
			return self::createIframe($matches[1], $convertStatus);
		}, $message, self::$_cvtimes);
	}
