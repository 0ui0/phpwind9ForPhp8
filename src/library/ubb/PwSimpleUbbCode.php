...
class PwSimpleUbbCode {
	...
	public static function convertTable($message, $max = 0) {
		...
		while (self::hasTag($message, 'table')) {
			$message = preg_replace_callback('/\[table(?:=(\d{1,4}(?:%|px)?)(?:,(#\w{6})?)?(?:,(#\w{6})?)?(?:,(\d+))?(?:,(\d+))?(?:,(left|center|right))?)?\](?!.*(\[table))(.*?)\[\/table\]/is', function($matches) {
				return self::_pushCode('createTable', $matches[8], $matches[1], $matches[2], $matches[3], $matches[4], $matches[5], $matches[6]);
			}, $message);
			if (++$t > $max) break;
		}
		return $message;
	...
	public static function parseEmotion($message) {
		$message = preg_replace_callback("/\[s:(.+?)\]/is", function($matches) {
			return self::_pushCode('createEmotion', $matches[1]);
		}, $message);
		return $message;
	}
	...
	public static function parseImg($message, $maxWidth = 0, $maxHeight = 0) {
		return preg_replace_callback("/\[img\]([^\<\r\n\"']+?)\[\/img\]/is", function($matches) use ($maxWidth, $maxHeight) {
			return self::_pushCode('createImg', $matches[1], $maxWidth, $maxHeight);
		}, $message);
	}
	...
	public static function parseUrl($message, $checkurl = 0) {
		$message = preg_replace_callback("/\[url=((https?|ftp|gopher|news|telnet|mms|rtsp|thunder)?[^\[\s]+?)(\,(1)\/?)?\](.+?)\[\/url\]/is", function($matches) use ($checkurl) {
			return self::_pushCode('createUrl', $matches[1], $matches[5], $matches[2], $matches[4], $checkurl);
		}, $message);

		$message = preg_replace_callback("/\[url\]((https?|ftp|gopher|news|telnet|mms|rtsp|thunder)?[^\[\s]+?)\[\/url\]/is", function($matches) use ($checkurl) {
			return self::_pushCode('createUrl', $matches[1], $matches[1], $matches[2], '0', $checkurl);
		}, $message);
	...
	public static function parseCode($message) {
		return preg_replace_callback("/\[code(\sbrush\:(.+?)\;toolbar\:(true|false)\;)?\](.+?)\[\/code\]/is", function($matches) {
			return self::_pushCode('createCode', $matches[4], $matches[2], $matches[3]);
		}, $message);
	}
	...
	public static function parsePost($message, $config) {
		return preg_replace_callback("/\[post\](.+?)\[\/post\]/is", function($matches) use ($config) {
			return self::_pushCode('createPost', $matches[1], $config);
		}, $message);
	}
	...
	public static function parseHide($message, $config) {
		return preg_replace_callback("/\[hide=(.+?)\](.+?)\[\/hide\]/is", function($matches) use ($config) {
			return self::_pushCode('createHide', $matches[1], $matches[2], $config);
		}, $message);
	}
	...
	public static function parseSell($message, $config) {
		return preg_replace_callback("/\[sell=(.+?)\](.+?)\[\/sell\]/is", function($matches) use ($config) {
			return self::_pushCode('createSell', $matches[1], $matches[2], $config);
		}, $message);
	}
	...
	public static function parseQuote($message) {
		return preg_replace_callback("/\[quote(=(.+?)\,\d+)?\](.*?)\[\/quote\]/is", function($matches) {
			return self::_pushCode('createQoute', $matches[3], $matches[2]);
		}, $message);
	}
	...
	public static function parseFlash($message, $config) {
		if ($config->isConvertFlash) {
			return preg_replace_callback("/\[flash(=(\d+?)\,(\d+?)(\,(0|1))?)?\]([^\[\<\r\n\"']+?)\[\/flash\]/is", function($matches) {
				return self::_pushCode('createPlayer', $matches[6], $matches[2], $matches[3], $matches[5], 'video');
			}, $message);
		}
		return preg_replace_callback("/\[flash(=(\d+?)\,(\d+?)(\,(0|1))?)?\]([^\[\<\r\n\"']+?)\[\/flash\]/is", function($matches) {
			return self::_pushCode('createFlashLink', $matches[6]);
		}, $message);
	}
	...
	public static function parseMedia($message, $config) {
		if ($config->isConvertMedia == 2) {
			$message = preg_replace_callback("/\[(wmv|mp3)(=(0|1))?\]([^\<\r\n\"']+?)\[\/\\1\]/is", function($matches) {
				return self::_pushCode('createPlayer', $matches[4], '314', '53', $matches[3], 'audio');
			}, $message);
			
			$message = preg_replace_callback("/\[(wmv|rm)(=([0-9]{1,3})\,([0-9]{1,3})\,(0|1))?\]([^\<\r\n\"']+?)\[\/\\1\]/is", function($matches) {
				return self::_pushCode('createPlayer', $matches[6], $matches[3], $matches[4], $matches[5], 'video');
			}, $message);
			return $message;
		}
		
		$message = preg_replace_callback("/\[(mp3|wmv)(?:=[01]{1})?\]([^\<\r\n\"']+?)\[\/\\1\]/is", function($matches) {
			 return self::_pushCode('createMediaLink', $matches[2]);
		}, $message);
	...
	public static function parseRemind($message, $remindUser) {
		return preg_replace_callback('/@([\x7f-\xff\dA-Za-z\.\_]+)(?=\s?)/i', function($matches) use ($remindUser) {
			return self::_pushCode('createRemind', $matches[1], $remindUser);
		}, $message);
	}
	...
	public static function parseIframe($message, $config) {
		return preg_replace_callback("/\[iframe\]([^\[\<\r\n\"']+?)\[\/iframe\]/is", function($matches) use ($config) {
			return self::_pushCode('createIframe', $matches[1], $config);
		}, $message);
	}
